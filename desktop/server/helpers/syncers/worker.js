    ...
    while (true) {
        ...
        if (record && record.node_id !== '') {
            ...
            if (remoteNodeResponse && (remoteNodeResponse.statusCode === 503 || remoteNodeResponse.statusCode === 500)) {
                ...
                logger.info(`Pausing ${pauseTime / 1000} seconds, since server load seems high. Status code: ${remoteNodeResponse.statusCode}`);
            ...
            if (remoteNodeResponse
                ...
                && record.download_in_progress === false
                && record.upload_in_progress === false) {
                logger.info(`Node ${record.node_id} not available on server, deleting on local: ${record.file_path} - ${record.id}`);
            ...
            if (remoteNodeResponse
                ...
                logger.info(`Node was moved to some other location, deleting on local: ${record.file_path} - ${record.id}`);
